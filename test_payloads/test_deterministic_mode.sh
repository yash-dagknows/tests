#!/bin/bash
# Test Deterministic Alert Handling Mode
# This script tests that a pre-configured task executes when a matching alert is received.

set -e

# Colors
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Load environment from .env if it exists
if [ -f "$(dirname "$0")/.env" ]; then
    export $(grep -v '^#' "$(dirname "$0")/.env" | xargs)
fi

# Configuration (with hardcoded defaults for dev.dagknows.com)
BASE_URL="${DAGKNOWS_URL:-https://dev.dagknows.com}"
TOKEN="${DAGKNOWS_TOKEN:-eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..IvHXxMMm4th-jm7IyektCTW3SXxlq4olv6gByzAliuFk1y2zbr91QBY_zdifz5_KnYNk4ruVr9xLrlahC3h7rGJaFU98OiiiLAQ49CFxgUO2iqyr6XcT3GgO5ptobTtOZNV4rmrPE_6hnjPRImernp5M6FhHhCZWktL9xQqZs9N9m2az1Jk-mi0yZLaNIA25pHWcqgdiF1xCbt5STWqFG2nmt1eq7x-FXnoryEqo1HvfOg74crBcubFORpTZ5AhNRpSdqemcpDd_6t0UliPuAg3VlwIedew9RBRvytr2j9GbBC_jgCh7YJ5oQqFjVItolhSMzuKHLRRHZib8Ikz6fA}"
# Critical: proxy parameter needed for dev.dagknows.com
PROXY_PARAM="${DAGKNOWS_PROXY:-?proxy=dev1}"

echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  Testing DETERMINISTIC Alert Handling Mode${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Target: $BASE_URL"
echo "Mode: Deterministic (pre-configured task triggers)"
echo ""

# Use a fixed alert name that user should configure in their task
ALERT_NAME="HighCPUUsage"

echo -e "${YELLOW}📝 Prerequisites:${NC}"
echo "  You should have a task configured with:"
echo "    • source: Grafana"
echo "    • alert_name: ${ALERT_NAME}"
echo "    • dedup_interval: 300"
echo ""
echo "  Set mode to 'deterministic' via UI: Settings → Alert Handling Mode"
echo ""
read -p "Press Enter to continue (or Ctrl+C to exit)..."
echo ""

# Step 1: Send matching Grafana alert
echo -e "${YELLOW}Step 1: Sending Grafana alert for ${ALERT_NAME}${NC}"
echo "  URL: ${BASE_URL}/processAlert${PROXY_PARAM}"
ALERT_RESPONSE=$(curl -s -X POST "${BASE_URL}/processAlert${PROXY_PARAM}" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${TOKEN}" \
    -d "{
        \"receiver\": \"Test_Endpoint\",
        \"status\": \"firing\",
        \"alerts\": [{
            \"status\": \"firing\",
            \"labels\": {
                \"alertname\": \"${ALERT_NAME}\",
                \"grafana_folder\": \"test\",
                \"instance\": \"test-server\",
                \"severity\": \"critical\"
            },
            \"annotations\": {
                \"description\": \"CPU usage exceeded 90% on test server\",
                \"summary\": \"High CPU Usage Alert\"
            },
            \"startsAt\": \"$(date -u +%s)\",
            \"fingerprint\": \"test$(date +%s)\"
        }],
        \"groupLabels\": {
            \"alertname\": \"${ALERT_NAME}\"
        },
        \"commonLabels\": {
            \"alertname\": \"${ALERT_NAME}\",
            \"severity\": \"critical\"
        },
        \"commonAnnotations\": {
            \"description\": \"CPU usage exceeded 90% on test server\",
            \"summary\": \"High CPU Usage Alert\"
        },
        \"externalURL\": \"http://grafana:3000/\",
        \"version\": \"1\",
        \"title\": \"[FIRING:1] ${ALERT_NAME}\",
        \"state\": \"alerting\"
    }")

echo "$ALERT_RESPONSE" | jq . 2>/dev/null || echo "$ALERT_RESPONSE"
echo ""

# Step 2: Verify execution
echo -e "${YELLOW}Step 2: Verifying execution${NC}"

STATUS=$(echo "$ALERT_RESPONSE" | jq -r '.status // "unknown"')
TASKS_EXECUTED=$(echo "$ALERT_RESPONSE" | jq -r '.tasks_executed // 0')
MESSAGE=$(echo "$ALERT_RESPONSE" | jq -r '.message // ""')

echo "  Status: ${STATUS}"
echo "  Tasks Executed: ${TASKS_EXECUTED}"
if [ -n "$MESSAGE" ]; then
    echo "  Message: ${MESSAGE}"
fi
echo ""

# Show results
if [ "$TASKS_EXECUTED" -ge 1 ]; then
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}  ✅ SUCCESS: Task(s) executed!${NC}"
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "Executed tasks:"
    echo "$ALERT_RESPONSE" | jq -r '.executed_tasks[]? | "  • Task ID: \(.task_id)\n    Job ID: \(.job_id)\n    Status: \(.status)"' 2>/dev/null || echo "  (Task executed successfully)"
    exit 0
else
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}  ⚠ No tasks executed${NC}"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "Possible reasons:"
    echo "  • No matching task configured with trigger"
    echo "  • Alert source or name mismatch"
    echo "  • Dedup interval blocking execution"
    echo "  • Mode not set correctly in UI"
    echo ""
    echo "Debug info:"
    echo "  Alert Name: ${ALERT_NAME}"
    echo "  Expected Source: Grafana"
    echo "  Detected Source: $(echo "$ALERT_RESPONSE" | jq -r '.alert_source')"
    echo ""
    echo "  Make sure you have a task configured with:"
    echo "    trigger_on_alerts: [{\"source\": \"Grafana\", \"alert_name\": \"${ALERT_NAME}\"}]"
    exit 0
fi

